...
//		驗證成功，session放入目前會員資料，然後多一頁跳轉方便過濾器刷新身份
		if (!status) {
//			驗證失敗，傳到失敗頁的會員資料和資訊
//			因為綁定form表單，所以要傳回member物件
			m.addAttribute("member", member);
...
//		驗證成功
		...
		// 會員驗證ok，接著建立cookie，搭配cookieFilter方便下次登入流程
		// 若勾選RememberMe**************************************
...
//	@ModelAttribute("thankU")代表在Model新增一個屬性，但是給值還是要透過m.addattribute
//	用傳進來的變數給值，效用只會在區域內
	@GetMapping("/35/logoutProcess")
	public String logoutProcess(@ModelAttribute("thankU") String thankU, Model m, ServletRequest request) {
